---
title: "study1_rentzsch2015_ck"
author: "Carson Kofler"
format: html
editor: visual
---
...
ggplot(data = data_complete, aes(x= game_condition_coded))+
  geom_bar(fill = "#39568CFF") +
  labs(
...
t.test(data=data_complete, state_envy_composite ~ game_condition_coded)

wilcox.test(data = data_complete, state_envy_composite ~ game_condition_coded)
...
#table write-out (t test)
t_envy_result <- t.test(data=data_complete, state_envy_composite ~ game_condition_coded)
apa_print(t_envy_result)$full_result
...
lm(state_envy_composite ~ total_dispositional_envy, data = subset(data_complete, game_condition_coded == 0)) #lose 

lm(state_envy_composite ~ total_dispositional_envy, data = subset(data_complete, game_condition_coded == 1)) #win
...
sapply(data_complete[75:77], function(x) cor(x, data_complete$state_envy_composite, use = "complete.obs"))
...
# scatter plot by domain type 
ggplot(data_complete_long, aes(x = disp_value, y = state_envy_composite, color = disp_type)) +
  geom_jitter(width = .2, height = .2, alpha = 0.7, size = 3) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(
    x = "Dispositional Envy",
    y = "State Envy Composite",
    color = "Dispositional Envy Domain",
    title = "State Envy by Dispositional Envy Domain"
  ) +
  theme_minimal(base_size = 12) +
  theme(plot.title = element_text(hjust = 0.5))

#write out (correlation)
apa.cor.table(data = data_complete[, c("state_envy_composite", "dispositional_envy_a", 
                                       "dispositional_envy_c", "dispositional_envy_w")], 
...
How do random chance resource distributions impact anger? Are people who lose by random chance more angry at winners?
...
Is there any bias in the data based on demand characteristics (dplyr::filter(data_complete, grepl('envy\|jealousy', type))?
...
#violin plot
ggplot(data_complete, aes(x = factor(game_condition_coded), y = state_anger_composite)) +
...
#Is the attribution of outcomes different depending on whether you win or lose? 
#Are demand characteristics biasing the results? (filter and sensitivity analysis)
detect <- "envy|envious|envies|jealous|jealousy|resentment"

funnel_questions <- c("funnel_1", "funnel_2", "comments")

# Clean columns: trim spaces and remove trailing punctuation
data_clean <- data_complete %>%
  mutate(across(all_of(funnel_questions), ~ str_trim(.) %>% str_replace_all("[[:punct:]]$", "")))

# Exclude rows containing keywords
data_excluding_demand <- data_clean %>%
  filter(if_all(all_of(funnel_questions), ~ !str_detect(., regex(detect, ignore_case = TRUE))))

n_distinct(data_complete) - n_distinct(data_excluding_demand) #number excluded 

summary(lm(data=data_excluding_demand, state_envy_composite ~ game_condition_coded))

t.test(x= data_excluding_demand$game_condition_coded, y = data_excluding_demand$state_envy_composite, 
